...
ImuState::ImuState(const ros::Time& time)
    : stamp_(time),
      orientation_(time, fuse_core::uuid::NIL),
      position_(time, fuse_core::uuid::NIL),
      velocity_(time, fuse_core::uuid::NIL),
      gyrobias_(time, fuse_core::uuid::NIL),
      accelbias_(time, fuse_core::uuid::NIL) {
...
bs_common::PreIntegrator& ImuState::GetPreintegratorMutable() {
  return preint_;
}

const bs_common::PreIntegrator ImuState::GetPreintegratorConst() const {
  return preint_;
}
