...
using namespace bs_common;  // allows short-form of ErrorStateLocation

class DeltaImuState3DCostFunction
 ...
 public:
   ...
   */
  DeltaImuState3DCostFunction(const bs_common::ImuState &imu_state_i,
  ...
  /**
   * @brief Override for Cere's Evaluate function (See Cere's documentation)
   */
  bool Evaluate(const double *const *parameters, double *residuals,
    ...
    // get state deltas
    const double &dt =
        ros::Duration(imu_state_j_.Stamp() - imu_state_i_.Stamp()).toSec();
    ...
    const Eigen::Vector3d dbg = bg_i - imu_state_i_.GyroBiasVec();
    const Eigen::Vector3d dba = ba_i - imu_state_i_.AccelBiasVec();
    ...
    if (jacobians) {
      ...
      if (jacobians[5]) {
        ...
        dr_dq_j.block<3, 3>(ES_Q, 0) =
            beam::RightJacobianOfSO3(r.segment<3>(ES_Q)).inverse();
        dr_dq_j.block<3, 3>(ES_P, 0) = -q_i.conjugate().matrix() * q_j.matrix();
 ...
 private:
  ...
  const bs_common::PreIntegrator &pre_integrator_;
};