...
RelocRefinementLoam::RelocRefinementLoam(const Eigen::Matrix<double, 6, 6>& reloc_covariance,
...
fuse_core::Transaction::SharedPtr
    RelocRefinementLoam::GenerateTransaction(const SubmapPtr& matched_submap,
                                             const SubmapPtr& query_submap,
                                             const Eigen::Matrix4d& T_MATCH_QUERY_EST) {
  // extract and filter clouds from matched submap
  LoamPointCloud matched_submap_in_submap_frame = matched_submap->GetLidarLoamPointsInWorldFrame();
  matched_submap_in_submap_frame.TransformPointCloud(
      beam::InvertTransform(matched_submap->T_WORLD_SUBMAP()));

  // extract and filter clouds from matched submap
  LoamPointCloud query_submap_in_submap_frame = query_submap->GetLidarLoamPointsInWorldFrame();
  ...
  if (!GetRefinedT_SUBMAP_QUERY(matched_submap_in_submap_frame, query_submap_in_submap_frame,
                                T_MATCH_QUERY_EST, T_MATCH_QUERY_OPT)) {
    return nullptr;
  }

  // create transaction
  bs_constraints::relative_pose::Pose3DStampedTransaction transaction(query_submap->Stamp());
  transaction.AddPoseConstraint(matched_submap->T_WORLD_SUBMAP(), query_submap->T_WORLD_SUBMAP(),
...
bool RelocRefinementLoam::GetRefinedPose(Eigen::Matrix4d& T_SUBMAP_QUERY_refined,
  ...
  LoamPointCloud submap_in_submap_frame = submap->GetLidarLoamPointsInWorldFrame();
  submap_in_submap_frame.TransformPointCloud(beam::InvertTransform(submap->T_WORLD_SUBMAP()));

  // get refined transform
  if (!GetRefinedT_SUBMAP_QUERY(submap_in_submap_frame, *loam_cloud_in_query_frame,
...
void RelocRefinementLoam::LoadConfig() {
  ...
  if (read_path == "DEFAULT_PATH") {
    read_path =
        bs_common::GetBeamSlamConfigPath() + "global_map/reloc_refinement_loam_registration.json";
  }
  ...
  } catch (...) { BEAM_ERROR("Missing one or more parameter, using default reloc params."); }
}
...
bool RelocRefinementLoam::GetRefinedT_SUBMAP_QUERY(const LoamPointCloud& submap_cloud,
  ...
  LoamPointCloudPtr query_in_submap_frame_est = std::make_shared<LoamPointCloud>(query_cloud);
  ...
  if (!matcher_->Match()) {
    ...
    if (output_results_) {
      output_path_stamped_ = debug_output_path_ +
                             beam::ConvertTimeToDate(std::chrono::system_clock::now()) + "_failed/";
  ...
  Eigen::Matrix4d T_SUBMAPREFINED_SUBMAPEST = matcher_->GetResult().inverse().matrix();
  ...
  if (output_results_) {
    output_path_stamped_ =
        debug_output_path_ + beam::ConvertTimeToDate(std::chrono::system_clock::now()) + "_passed/";
    boost::filesystem::create_directory(output_path_stamped_);
    matcher_->SaveResults(output_path_stamped_);
  }
