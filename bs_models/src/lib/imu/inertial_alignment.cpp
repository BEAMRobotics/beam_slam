...
double ImuObservability(const std::vector<bs_common::ImuState>& imu_frames) {
  ...
  for (size_t i = 0; i < imu_frames.size(); ++i) {
    ...
    const Eigen::Vector3d tmp_g = imu_frames[i].GetConstPreintegrator().delta.v / dt;
    sum_g += tmp_g;
  }
...
void EstimateGyroBias(const std::vector<bs_common::ImuState>& imu_frames, Eigen::Vector3d& bg) {
  const int N = imu_frames.size();
  Eigen::Matrix3d A = Eigen::Matrix3d::Zero();
  Eigen::Vector3d b = Eigen::Vector3d::Zero();
  for (size_t j = 1; j < N; ++j) {
    const size_t i = j - 1;
    const Eigen::Quaterniond& dq = imu_frames[j].GetConstPreintegrator().delta.q;
    const Eigen::Matrix3d& dq_dbg = imu_frames[j].GetConstPreintegrator().jacobian.dq_dbg;
...
void EstimateGravityScaleVelocities(const std::vector<bs_common::ImuState>& imu_frames,
  ...
  for (size_t j = 1; j < N; ++j) {
    ...
    const bs_common::Delta& delta_ij = imu_frames[j].GetConstPreintegrator().delta;
  ...
  scale = x(3);
  for (size_t i = 0; i < N; ++i) { velocities[i].second = x.segment<3>(4 + i * 3); }
}

void RefineGravityScaleVelocities(const std::vector<bs_common::ImuState>& imu_frames,
                                  Eigen::Vector3d& gravity, double& scale,
                                  std::vector<std::pair<uint64_t, Eigen::Vector3d>>& velocities) {
  const int N = imu_frames.size();
  ...
  for (size_t j = 1; j < N; ++j) {
    ...
    b.segment<3>(i * 6 + 3) = delta.t.toSec() * gravity + imu_frames[i].OrientationQuat() * delta.v;
  ...
  scale = x(2);
  for (size_t i = 0; i < N; ++i) { velocities[i].second = x.segment<3>(3 + i * 3); }
}
