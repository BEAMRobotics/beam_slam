...
void GlobalMap::Params::LoadJson(const std::string& config_path) {
  ...
  if (read_file.empty()) {
    BEAM_INFO("No config file provided to global map, using default parameters.");
    return;
  }

  if (read_file == "DEFAULT_PATH") {
    read_file = bs_common::GetBeamSlamConfigPath() + "global_map/global_map.json";
  }
  ...
  if (vec.size() != 6) {
    BEAM_ERROR("Invalid local mapper covariance diagonal (6 values required). Using "
               "default.");
  } else {
...
void GlobalMap::Params::SaveJson(const std::string& filename) {
  nlohmann::json J = {{"submap_size_m", submap_size},
                      ...
                      {"reloc_covariance_diag",
                       {reloc_covariance(0, 0), reloc_covariance(1, 1), reloc_covariance(2, 2),
                        reloc_covariance(3, 3), reloc_covariance(4, 4), reloc_covariance(5, 5)}}};

  std::ofstream file(filename);
  file << std::setw(4) << J << std::endl;
}

GlobalMap::GlobalMap(const std::shared_ptr<beam_calibration::CameraModel>& camera_model,
...
GlobalMap::GlobalMap(const std::string& data_root_directory) {
  if (!Load(data_root_directory)) {
    ...
    throw std::runtime_error{"Unable to instantiate GlobalMap with input data."};
  }
}
...
const std::shared_ptr<beam_cv::ImageDatabase>& GlobalMap::GetOnlineImageDatabase() {
  return online_image_database_;
}

const std::shared_ptr<beam_cv::ImageDatabase>& GlobalMap::GetOfflineImageDatabase() {
...
void GlobalMap::SetOnlineImageDatabase(std::shared_ptr<beam_cv::ImageDatabase> image_db) {
  online_image_database_ = image_db;
}

void GlobalMap::SetOfflineImageDatabase(std::shared_ptr<beam_cv::ImageDatabase> image_db) {
...
void GlobalMap::Setup() {
  ...
  if (params_.reloc_refinement_type == "ICP") {
    reloc_refinement_ = std::make_unique<RelocRefinementIcp>(params_.reloc_covariance,
                                                             params_.reloc_refinement_config);
  } else if (params_.reloc_refinement_type == "GICP") {
    reloc_refinement_ = std::make_unique<RelocRefinementGicp>(params_.reloc_covariance,
                                                              params_.reloc_refinement_config);
  } else if (params_.reloc_refinement_type == "NDT") {
    reloc_refinement_ = std::make_unique<RelocRefinementNdt>(params_.reloc_covariance,
                                                             params_.reloc_refinement_config);
  } else if (params_.reloc_refinement_type == "LOAM") {
    reloc_refinement_ = std::make_unique<RelocRefinementLoam>(params_.reloc_covariance,
                                                              params_.reloc_refinement_config);
  } else {
...
fuse_core::Transaction::SharedPtr
  ...
  if (submap_id == online_submaps_.size()) {
    SubmapPtr new_submap =
        std::make_shared<Submap>(stamp, T_WORLD_BASELINK, camera_model_, extrinsics_);
    ...
    if (reloc_transaction != nullptr) { new_transaction->merge(*reloc_transaction); }
  ...
  if (!cam_measurement.landmarks.empty()) {
    ...
    online_submaps_.at(submap_id)->AddCameraMeasurement(cam_measurement, T_WORLD_BASELINK);
  }

  // if lidar measurement exists, check frame id
  if (!lid_measurement.lidar_points.empty() || !lid_measurement.lidar_edges_strong.empty() ||
      !lid_measurement.lidar_surfaces_strong.empty()) {
    if (lid_measurement.frame_id != extrinsics_->GetLidarFrameId()) {
      BEAM_WARN("Lidar measurement frame id not consistent with lidar frame in the "
                "extrinsics class.");
    }
  }
  ...
  if (!traj_measurement.stamps.empty()) {
    ...
    if (poses_vec.size() % 12 != 0) {
      ...
      BEAM_ERROR("Number of poses is not equal to number of time stamps. Not adding "
      ...
      for (int i = 0; i < num_poses; i++) {
        ...
        for (int j = 0; j < 12; j++) { current_pose.push_back(traj_measurement.poses[12 * i + j]); }
        Eigen::Matrix4d T_KEYFRAME_FRAME = beam::VectorToEigenTransform(current_pose);
        std::string matrix_check_summary;
        if (!beam::IsTransformationMatrix(T_KEYFRAME_FRAME, matrix_check_summary)) {
      ...
      online_submaps_.at(submap_id)->AddTrajectoryMeasurement(poses, stamps, stamp);
    }
  }
...
int GlobalMap::GetSubmapId(const Eigen::Matrix4d& T_WORLD_BASELINK) {
  ...
  Eigen::Vector3d t_WORLD_SUBMAPCUR =
      online_submaps_.at(online_submaps_.size() - 1)->T_WORLD_SUBMAP_INIT().block(0, 3, 3, 1);
  ...
  Eigen::Vector3d t_WORLD_SUBMAPPREV =
      online_submaps_.at(online_submaps_.size() - 2)->T_WORLD_SUBMAP_INIT().block(0, 3, 3, 1);
...
fuse_core::Transaction::SharedPtr GlobalMap::InitiateNewSubmapPose() {
  ...
  const SubmapPtr& current_submap = online_submaps_.at(online_submaps_.size() - 1);
  bs_constraints::relative_pose::Pose3DStampedTransaction new_transaction(current_submap->Stamp());
  new_transaction.AddPoseVariables(current_submap->Position(), current_submap->Orientation(),
                                   current_submap->Stamp());

  // if first submap, add prior then return
  if (online_submaps_.size() == 1) {
    new_transaction.AddPosePrior(current_submap->Position(), current_submap->Orientation(),
                                 pose_prior_noise_, "FIRSTSUBMAPPRIOR");
    return new_transaction.GetTransaction();
  }

  // If not first submap add constraint to previous
  const SubmapPtr& previous_submap = online_submaps_.at(online_submaps_.size() - 2);

  Eigen::Matrix4d T_PREVIOUS_CURRENT =
      beam::InvertTransform(previous_submap->T_WORLD_SUBMAP()) * current_submap->T_WORLD_SUBMAP();

  std::string source = "LOCALMAPPER";
  new_transaction.AddPoseConstraint(
      previous_submap->T_WORLD_SUBMAP(), current_submap->T_WORLD_SUBMAP(), previous_submap->Stamp(),
      current_submap->Stamp(), T_PREVIOUS_CURRENT, params_.local_mapper_covariance, source);
...
fuse_core::Transaction::SharedPtr GlobalMap::RunLoopClosure(int query_index) {
  ...
  const Eigen::Matrix4d& T_WORLD_QUERY = online_submaps_.at(query_index)->T_WORLD_SUBMAP();

  std::vector<cv::Mat> query_images = online_submaps_.at(query_index)->GetKeyframeVector();
  ...
  ROS_DEBUG("Found %d loop closure candidates.", matched_indices_filtered.size());

  if (matched_indices_filtered.size() == 0) { return nullptr; }

  ROS_DEBUG("Matched index[0]: %d, Query Index: %d, No. or submaps: %d. Running loop "
            "closure refinement",
  ...
  for (int i = 0; i < matched_indices_filtered.size(); i++) {
    ...
    fuse_core::Transaction::SharedPtr new_transaction =
        reloc_refinement_->GenerateTransaction(online_submaps_.at(matched_indices_filtered[i]),
...
bool GlobalMap::ProcessRelocRequest(const RelocRequestMsg& reloc_request_msg,
  ...
  if (!beam::IsTransformationMatrix(T_WORLDLM_QUERY, matrix_check_summary)) {
    BEAM_WARN("transformation matrix invalid, not running reloc. Reason: %s, Input:",
              matrix_check_summary.c_str());
  ...
  beam_matching::LoamPointCloudPtr loam_cloud_in_query_frame =
      std::make_shared<beam_matching::LoamPointCloud>(
          beam::ROSVectorToPCL(reloc_request_msg.lidar_measurement.lidar_edges_strong),
          beam::ROSVectorToPCL(reloc_request_msg.lidar_measurement.lidar_surfaces_strong),
          beam::ROSVectorToPCL(reloc_request_msg.lidar_measurement.lidar_edges_weak),
          beam::ROSVectorToPCL(reloc_request_msg.lidar_measurement.lidar_surfaces_weak));

  // if either lidar clouds have points, check the frame id
  if (!lidar_cloud_in_query_frame.empty() || !loam_cloud_in_query_frame->Empty()) {
    if (reloc_request_msg.lidar_measurement.frame_id != extrinsics_->GetBaselinkFrameId()) {
      BEAM_WARN("Frame id of lidar data in reloc request not consistent with the "
  ...
  // load image
  cv::Mat query_image;
  // check that reloc refinement is visual, otherwise don't bother
  if (!reloc_request_msg.camera_measurement.image.data.empty()) {
    query_image =
        beam_cv::OpenCVConversions::RosImgToMat(reloc_request_msg.camera_measurement.image);
  }
  ...
  auto process_submaps = [&](const auto& submaps, const auto& T_WORLD_QUERY,
    ...
    for (uint16_t i = 0; i < matched_indices.size(); i++) {
      ...
      if (active_submap_id_ == submap_index && active_submap_type_ == submap_type) {
      ...
      // get refined pose
      ROS_DEBUG("Looking for refined submap pose within submap index: ", submap_index);
      const auto& submap = submaps.at(submap_index);
      Eigen::Matrix4d T_SUBMAP_QUERY_refined;
      if (reloc_refinement_->GetRefinedPose(T_SUBMAP_QUERY_refined, T_SUBMAP_QUERY_initial, submap,
        ...
        if (!T_WORLDLM_WORLDOFF_found_ && submap_type == SubmapType::OFFLINE) {
          ROS_DEBUG("Setting transform from offline map world frame to local mapper "
                    "world frame.");
          T_WORLDLM_WORLDOFF_ = T_WORLD_QUERY * beam::InvertTransform(T_SUBMAP_QUERY_refined) *
                                beam::InvertTransform(submap->T_WORLD_SUBMAP());
          T_WORLDLM_WORLDOFF_found_ = true;
        }

        // get all required submap data
        PointCloud lidar_cloud_in_woff_frame = submap->GetLidarPointsInWorldFrameCombined(false);
        beam_matching::LoamPointCloud loam_cloud_in_woff_frame =
            submap->GetLidarLoamPointsInWorldFrame(false);
        PointCloud keypoints_in_woff_frame = submap->GetKeypointsInWorldFrame(false);
        ...
        FillSubmapMsg(submap_msg, lidar_cloud_in_woff_frame, loam_cloud_in_woff_frame,
                      keypoints_in_woff_frame, words, word_ids, submap->DescriptorType(),
                      T_WORLDLM_WORLDOFF_);

        // set current submap
        active_submap_id_ = submap_index;
        active_submap_type_ = submap_type;
        return true;
      } else {
...
void GlobalMap::SaveData(const std::string& output_path) {
  if (!boost::filesystem::exists(output_path)) {
    BEAM_ERROR("Global map output path does not exist, not saving map. Input: {}", output_path);
    return;
  }
  ...
  online_image_database_->SaveDatabase(output_path + "image_database.dbow3",
                                       output_path + "image_db_timestamps.json");
...
bool GlobalMap::Load(const std::string& root_directory) {
  if (!boost::filesystem::exists(root_directory)) {
    BEAM_ERROR("Global map root directory path does not exist, not loading map. "
               "Input: {}",
               root_directory);
    return false;
  }
  ...
  if (!boost::filesystem::exists(root_directory + "params.json")) {
    BEAM_ERROR("params.json not foudn in root directory, not loading GlobalMap. "
  ...
  if (!boost::filesystem::exists(root_directory + "camera_model.json")) {
    BEAM_ERROR("camera_model.json not found in root directory, not loading GlobalMap. "
               "Input root directory: {}",
               root_directory);
    return false;
  }
  ...
  if (!boost::filesystem::exists(root_directory + "image_database.dbow3") ||
    ...
    online_image_database_ = std::make_shared<beam_cv::ImageDatabase>(
        root_directory + "image_database.dbow3", root_directory + "image_db_timestamps.json");
  }
  ...
  int submap_num = 0;
  while (true) {
    std::string submap_dir = root_directory + "submap" + std::to_string(submap_num) + "/";
    if (!boost::filesystem::exists(submap_dir)) { break; }

    SubmapPtr current_submap = std::make_shared<Submap>(ros::Time(0), Eigen::Matrix4d::Identity(),
...
void GlobalMap::SaveLidarSubmaps(const std::string& output_path, bool save_initial) {
  if (!boost::filesystem::exists(output_path)) {
    BEAM_ERROR("Invalid output path, not saving submaps. Input: {}", output_path);
    return;
  }
  ...
  for (int i = 0; i < online_submaps_.size(); i++) {
    std::string submap_name = submaps_path + "lidar_submap" + std::to_string(i) + ".pcd";
    online_submaps_.at(i)->SaveLidarMapInWorldFrame(submap_name, max_output_map_size_);
  }
  ...
  for (int i = 0; i < online_submaps_.size(); i++) {
    std::string submap_name = submaps_path_initial + "lidar_submap" + std::to_string(i) + ".pcd";
    online_submaps_.at(i)->SaveLidarMapInWorldFrame(submap_name, max_output_map_size_, true);
  }
}

void GlobalMap::SaveKeypointSubmaps(const std::string& output_path, bool save_initial) {
  if (!boost::filesystem::exists(output_path)) {
    BEAM_ERROR("Invalid output path, not saving submaps. Input: {}", output_path);
    return;
  }
  ...
  boost::filesystem::create_directory(submaps_path);
  for (int i = 0; i < online_submaps_.size(); i++) {
    std::string submap_name = submaps_path + "keypoint_submap" + std::to_string(i) + ".pcd";
  ...
  boost::filesystem::create_directory(submaps_path_initial);
  for (int i = 0; i < online_submaps_.size(); i++) {
    std::string submap_name = submaps_path_initial + "keypoint_submap" + std::to_string(i) + ".pcd";
    online_submaps_.at(i)->SaveKeypointsMapInWorldFrame(submap_name, true);
  }
}

void GlobalMap::SaveTrajectoryFile(const std::string& output_path, bool save_initial) {
  if (!boost::filesystem::exists(output_path)) {
    BEAM_ERROR("Invalid output path, not saving trajectory file. Input: {}", output_path);
    return;
  }
  ...
  // save
  std::string output_file = output_path + "global_map_trajectory_optimized.json";
  ...
  // save
  std::string output_file_initial = output_path + "global_map_trajectory_initial.json";
  poses_initial.WriteToJSON(output_file_initial);
}

void GlobalMap::SaveTrajectoryClouds(const std::string& output_path, bool save_initial) {
  if (!boost::filesystem::exists(output_path)) {
    BEAM_ERROR("Invalid output path, not saving trajectory clouds. Input: {}", output_path);
    return;
  }
  ...
  if (!beam::SavePointCloud<pcl::PointXYZRGBL>(
          output_file, cloud, beam::PointCloudFileType::PCDBINARY, error_message)) {
  ...
  if (!beam::SavePointCloud<pcl::PointXYZRGBL>(output_path + "global_map_trajectory_initial.pcd",
                                               cloud_initial, beam::PointCloudFileType::PCDBINARY,
                                               error_message)) {
    BEAM_ERROR("Unable to save trajectory cloud. Reason: {}", error_message);
  }
}

void GlobalMap::SaveSubmapFrames(const std::string& output_path, bool save_initial) {
  if (!boost::filesystem::exists(output_path)) {
    BEAM_ERROR("Invalid output path, not saving submap frames. Input: {}", output_path);
    return;
  }
  ...
  for (uint16_t i = 0; i < online_submaps_.size(); i++) {
    const SubmapPtr& submap = online_submaps_.at(i);
    pcl::PointCloud<pcl::PointXYZRGBL> frame = beam::CreateFrameCol(submap->Stamp());
  ...
  std::string output_file = output_path + "global_map_submap_poses_optimized.pcd";
  ...
  for (uint16_t i = 0; i < online_submaps_.size(); i++) {
    ...
    pcl::PointCloud<pcl::PointXYZRGBL> frame = beam::CreateFrameCol(submap->Stamp());
    pcl::PointCloud<pcl::PointXYZRGBL> frame_transformed;
    pcl::transformPointCloud(frame, frame_transformed, submap->T_WORLD_SUBMAP_INIT());
    cloud_initial += frame_transformed;
  }
  std::string output_file_initial = output_path + "global_map_submap_poses_initial.pcd";
  BEAM_INFO("Saving submap frames to: {}", output_file_initial);
  if (!beam::SavePointCloud<pcl::PointXYZRGBL>(
          output_file_initial, cloud_initial, beam::PointCloudFileType::PCDBINARY, error_message)) {
...
void GlobalMap::AddRosSubmap(int submap_id) {
  ...
  PointCloud new_submap_pcl_cloud = submap_ptr->GetLidarPointsInWorldFrameCombined(true);

  sensor_msgs::PointCloud2 pointcloud2_msg;

  if (new_submap_pcl_cloud.size() > 0) {
    // filter cloud
    new_submap_pcl_cloud =
        beam_filtering::FilterPointCloud(new_submap_pcl_cloud, params_.ros_submap_filter_params);

    // convert to PointCloud2
    pointcloud2_msg = beam::PCLToROS(new_submap_pcl_cloud, submap_ptr->Stamp(),
...
void GlobalMap::AddRosGlobalMap() {
  ...
  for (const auto& submap_ptr : online_submaps_) {
    ...
    std::vector<PointCloud> new_submap_points = submap_ptr->GetLidarPointsInWorldFrame(10e6, false);
    for (const PointCloud& cloud : new_submap_points) { new_submap_pcl_cloud += cloud; }
  ...
  if (global_lidar_map.size() > 0) {
    // filter global map
    global_lidar_map =
        beam_filtering::FilterPointCloud(global_lidar_map, params_.ros_globalmap_filter_params);

    // convert lidar map to PointCloud2
    sensor_msgs::PointCloud2 pointcloud2_msg = beam::PCLToROS(
        global_lidar_map, last_update_time_, extrinsics_->GetWorldFrameId(), global_map_updates_);
  ...
  if (global_keypoints_map.size() > 0) {
    // convert lidar map to PointCloud2
    sensor_msgs::PointCloud2 pointcloud2_msg =
        beam::PCLToROS(global_keypoints_map, last_update_time_, extrinsics_->GetWorldFrameId(),
                       global_map_updates_);
...
void GlobalMap::AddNewRosScan(const PointCloud& cloud, const Eigen::Matrix4d& T_WORLD_BASELINK,
  ...
  // convert to PointCloud2
  sensor_msgs::PointCloud2 pointcloud2_msg = beam::PCLToROS(
      cloud_in_world_frame, stamp, extrinsics_->GetWorldFrameId(), new_scans_counter_);
  new_scans_counter_++;
...
void GlobalMap::FillSubmapMsg(SubmapMsg& submap_msg, const PointCloud& lidar_points,
                              ...
                              const std::string& descriptor_type, const Eigen::Matrix4d& T) const {
  ...
  if (!T.isIdentity()) {
    pcl::transformPointCloud(lidar_points_in_wlm_frame, lidar_points_in_wlm_frame, T);
