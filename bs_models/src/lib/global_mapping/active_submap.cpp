...
ActiveSubmap::ActiveSubmap() {
  ...
  // setup subsriber
  submap_subscriber_ =
      n.subscribe("/global_mapper/active_submap", 1, &ActiveSubmap::ActiveSubmapCallback, this);

  // setup publishers
  visual_map_publisher_ = n.advertise<sensor_msgs::PointCloud2>("/active_submap/visual_map", 10);
  lidar_map_publisher_ = n.advertise<sensor_msgs::PointCloud2>("/active_submap/lidar_map", 10);
  loam_edges_strong_publisher_ =
      n.advertise<sensor_msgs::PointCloud2>("/active_submap/loam_map/edges_strong", 10);
  loam_edges_weak_publisher_ =
      n.advertise<sensor_msgs::PointCloud2>("/active_submap/loam_map/edges_weak", 10);
  loam_surfaces_strong_publisher_ =
      n.advertise<sensor_msgs::PointCloud2>("/active_submap/loam_map/surfaces_strong", 10);
  loam_surfaces_weak_publisher_ =
      n.advertise<sensor_msgs::PointCloud2>("/active_submap/loam_map/surfaces_weak", 10);
...
void ActiveSubmap::ActiveSubmapCallback(const bs_common::SubmapMsg::ConstPtr& msg) {
  updates_counter_++;
  ...
  if (!msg->lidar_map.lidar_points.empty() || !msg->lidar_map.lidar_edges_strong.empty() ||
      !msg->lidar_map.lidar_surfaces_strong.empty()) {
    if (msg->lidar_map.frame_id != extrinsics_online_.GetWorldFrameId()) {
      BEAM_WARN("Lidar measurement frame id in submap msg not consistent with world "
                "frame in extrinsics.");
    }
  }
  ...
  for (geometry_msgs::Vector3 point_vec : msg->lidar_map.lidar_surfaces_strong) {
    pcl::PointXYZ point(point_vec.x, point_vec.y, point_vec.z);
  ...
  loam_cloud_ = std::make_shared<beam_matching::LoamPointCloud>(edges_strong, surfaces_strong,
...
std::vector<Eigen::Vector3d>
    ActiveSubmap::GetVisualMapVectorInCameraFrame(const Eigen::Matrix4d& T_WORLD_CAMERA) const {
  PointCloud cloud_in_cam_frame = GetVisualMapCloudInCameraFrame(T_WORLD_CAMERA);

  std::vector<Eigen::Vector3d> vector_in_cam_frame;
  for (auto it = visual_map_points_->begin(); it != visual_map_points_->end(); it++) {
...
void ActiveSubmap::Publish() const {
  ...
  if (!visual_map_points_->empty()) {
    sensor_msgs::PointCloud2 pc_msg =
        beam::PCLToROS(*visual_map_points_, update_time_, frame_id, updates_counter_);
    visual_map_publisher_.publish(pc_msg);
  }

  if (!lidar_map_points_->empty()) {
    sensor_msgs::PointCloud2 pc_msg =
        beam::PCLToROS(*lidar_map_points_, update_time_, frame_id, updates_counter_);
    lidar_map_publisher_.publish(pc_msg);
  }

  if (!loam_cloud_->edges.strong.cloud.empty()) {
    sensor_msgs::PointCloud2 pc_msg =
        beam::PCLToROS(loam_cloud_->edges.strong.cloud, update_time_, frame_id, updates_counter_);
    loam_edges_strong_publisher_.publish(pc_msg);
  }

  if (!loam_cloud_->edges.weak.cloud.empty()) {
    sensor_msgs::PointCloud2 pc_msg =
        beam::PCLToROS(loam_cloud_->edges.weak.cloud, update_time_, frame_id, updates_counter_);
    loam_edges_weak_publisher_.publish(pc_msg);
  }

  if (!loam_cloud_->surfaces.strong.cloud.empty()) {
    sensor_msgs::PointCloud2 pc_msg = beam::PCLToROS(loam_cloud_->surfaces.strong.cloud,
  ...
  if (!loam_cloud_->surfaces.weak.cloud.empty()) {
    sensor_msgs::PointCloud2 pc_msg =
        beam::PCLToROS(loam_cloud_->surfaces.weak.cloud, update_time_, frame_id, updates_counter_);
