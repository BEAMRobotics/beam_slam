...
void IMUInitializer::SolveAccelBias() {
  ...
  for (size_t j = 1; j + 1 < frames_.size(); ++j) {
    ...
    const bs_common::Delta& delta_ij = frames_[j].preint.delta;
    const bs_common::Delta& delta_jk = frames_[k].preint.delta;
    const bs_common::Jacobian& jacobian_ij = frames_[j].preint.jacobian;
    const bs_common::Jacobian& jacobian_jk = frames_[k].preint.jacobian;
...
void IMUInitializer::RefineGravityAndScale() {
  ...
  for (size_t iter = 0; iter < 5; ++iter) {
    ...
    for (size_t j = 1; j < frames_.size(); ++j) {
      ...
      const bs_common::Delta& delta = frames_[j].preint.delta;
